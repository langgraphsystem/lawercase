...
"""
        ...
        return CriterionEvaluation(
            ...
            recommendations=[
                (
                    ...
                    if evidence_count > 0
                    else "Gather evidence for this criterion"
                )
            ],
        )
    ...
    def _generate_recommendation(self, result: CriteriaAnalysisResult) -> str:
        ...
        if criteria_met >= 5:
            return (
                "STRONG CASE - Recommend filing. Exceeds minimum requirements with robust evidence."
            )
        if criteria_met >= 3:
    ...
    def _identify_strengths(self, result: CriteriaAnalysisResult) -> list[str]:
        ...
        for criterion, evaluation in result.criteria_evaluations.items():
            if evaluation.overall_strength in [
                EvidenceStrength.STRONG,
                EvidenceStrength.EXCEPTIONAL,
            ]:
                strengths.append(
                    f"{self.CRITERION_DESCRIPTIONS[criterion]['en']}: " f"{evaluation.summary}"
                )
    ...
    def _generate_next_steps(self, result: CriteriaAnalysisResult) -> list[str]:
        ...
        if len(result.criteria_met) < 3:
            next_steps.insert(
                0,
                "CRITICAL: Need to meet at least 3 criteria. Focus on gathering additional evidence.",
            )
    ...
    def get_all_criteria_info(self) -> dict[CriterionType, dict[str, Any]]:
        ...
        return {criterion: self.get_criterion_info(criterion) for criterion in CriterionType}
