...
class StorageConfig(BaseSettings):
    ...
    r2_access_key_id: SecretStr = Field(..., description="R2 access key ID (S3-compatible)")
    r2_secret_access_key: SecretStr = Field(..., description="R2 secret access key (S3-compatible)")
    r2_bucket_name: str = Field(
        default="mega-agent-documents", description="R2 bucket name for document storage"
    )
    r2_endpoint: str = Field(
        ..., description="R2 endpoint URL (https://<account-id>.r2.cloudflarestorage.com)"
    )
